...
export function useStudentService() {
    ...
    const createStudent = async (student: Omit<Student, "id">): Promise<Student> => {
        try {
            const response = await axios.post<Student>(`${API_URL}/create-employee`, student, {
                headers: {
                    "Content-Type": "application/json",
                    "api-key": API_KEY,
                    ...(student && (student as any).id ? { "student-id": (student as any).id } : {}),
                },
            });
            return response.data;
        } catch (error) {
    ...
    const updateStudent = async (id: number, student: Partial<Student>): Promise<Student> => {
        try {
            const response = await axios.put<Student>(`${API_URL}/update-employee`, student, {
                headers: {
                    "Content-Type": "application/json",
                    "api-key": API_KEY,
                    "student-id": id,
